...
rule cluster:
    input: 
        'data/processed/{study}/all_features_clean.csv'
    output:
        clusters = 'out/clusters/{study}/{cluster_num}/{algo}_cluster.csv',
        centroids = 'out/clusters/{study}/{cluster_num}/{algo}_cluster_centroids.csv',
        covariances = 'out/clusters/{study}/{cluster_num}/{algo}_cluster_covariances.csv'
    params:
        run_selection = False,
        features = lambda w: config["features"]["{}".format(w.study)],
        cluster_settings = lambda w: config["cluster_settings"]["{}".format(w.study)]
