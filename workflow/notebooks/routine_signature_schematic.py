...
ax_a.legend(
    handles, labels, loc="upper right", fontsize=9, framealpha=0.95, edgecolor="gray"
)

ax_a.set_title(
    "A.   Input Behavioral Data", loc="left", fontsize=14, fontweight="bold", pad=10
)
...
bars = ax_c.bar(
    range(1, K + 1), expected_days, color=colors, edgecolor="none", width=0.85
)
...
ax_c.set_title(
    "C.   Expected Number of Days per Cluster",
    loc="left",
    fontsize=14,
    fontweight="bold",
    pad=10,
...
ax_d.scatter(
    range(1, K + 1), signature, s=60, color="#3B6FA8", edgecolor="none", zorder=3
)
...
ax_d.set_title(
    "D.   Routine Signature", loc="left", fontsize=14, fontweight="bold", pad=10
)
...
plt.savefig(
    "/mnt/user-data/outputs/routine_signature_schematic.pdf",
    bbox_inches="tight",
    facecolor="white",
)
